...
const RPMManager: React.FC<RPMManagerProps> = ({ user, onNavigate }) => {
  ...
  const handleGenerateAsesmenAI = async (id: string) => {
    ...
    try {
      const result = await generateAssessmentDetails(rpm.tujuanPembelajaran, rpm.materi, rpm.kelas, context, rpm.jumlahPertemuan || 1);
  ...
  const syncWithPromes = async (id: string, promesId: string) => {
    ...
    // PEMBARUAN: Mengambil DPL langsung dari ATP jika ada, jika tidak gunakan keyword matching sebagai fallback
    let selectedDimensi: string[] = [];
    
    if (atp && atp.dimensiProfilLulusan) {
      const atpDplText = atp.dimensiProfilLulusan.toLowerCase();
      // Bandingkan string DPL dari ATP dengan daftar resmi DIMENSI_PROFIL
      ...
      // Fallback: Keyword matching jika atpId tidak ditemukan atau kosong
      const rawText = (
    ...
    try {
      await updateDoc(doc(db, "rpm", id), {
        ...
        // PEMBARUAN: Mengambil Alokasi Waktu langsung dari Program Semester
  ...
  const stripFilosofiTags = (text: string) => {
    if (!text) return '';
    // Regex untuk menghapus tag dalam kurung siku dan juga variasi teksnya
